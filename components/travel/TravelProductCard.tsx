...
import { Trash2, Bitcoin, IndianRupee, Clock, MapPin, Utensils, X } from "lucide-react";
import Image from "next/image";
import { useRouter } from "next/navigation";

// Custom Ethereum logo
const EthereumLogo = ({ size = 20 }: { size?: number }) => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 417" width={size} height={size}>
    <path fill="#343434" d="M127.9 0L124 13.1v272.6l3.9 3.9 127.9-75.6z" />
    <path fill="#8C8C8C" d="M127.9 0L0 214l127.9 75.6V154.1z" />
    <path fill="#3C3C3B" d="M127.9 312.1l-2.2 2.7v101.1l2.2 6.2 128-180.4z" />
    <path fill="#8C8C8C" d="M127.9 422.1V312.1L0 241.7z" />
    <path fill="#141414" d="M127.9 289.6l127.9-75.6-127.9-59.9z" />
    <path fill="#393939" d="M0 214l127.9 75.6v-135.5z" />
  </svg>
);
...
export default function TravelProductCard({ productId, onDelete }: TravelProductCardProps) {
  ...
  const handleBookNow = () => {
    ...
    router.push(
      `/TravelCheckout?productId=${productId}&category=${encodeURIComponent(
            ...
            {categories.length > 0 && (
              <div className="flex flex-col gap-1">
                ...
                <div className="flex items-center gap-1 text-sm text-gray-500">
                  <Bitcoin size={16} /> {categories[0].pricing.btc}
                </div>
                <div className="flex items-center gap-1 text-sm text-gray-500">
                  <EthereumLogo size={16} /> {categories[0].pricing.eth}
                </div>
              </div>
            )}
          </div>
                ...
                >
                  ...
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div className="flex items-center gap-1 text-sm text-gray-600">
                      <Bitcoin size={16} /> {cat.pricing.btc}
                    </div>
                    <div className="flex items-center gap-1 text-sm text-gray-600">
                      <EthereumLogo size={16} /> {cat.pricing.eth}
                    </div>
                  </div>
